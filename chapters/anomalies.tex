...
However, this only applies to very high-scoring objects above $\sim5\sigma$, and is likely a result of how we compute the raw scores{\emdash}there is a maximum to the generator residual due to the pixel values, while discriminator scores are essentially unbounded{\emdash}so we do not subscribe great significance to this.
